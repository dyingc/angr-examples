...
def test_ais3_crackme():
    exampletest_single("ais3_crackme")


@slow_test
def test_asisctffinals2015_fake():
    exampletest_single("asisctffinals2015_fake")


def test_asisctffinals2015_license():
    exampletest_single("asisctffinals2015_license")


@slow_test
def test_CADET_00001():
    exampletest_single("CADET_00001")  # 2m


@slow_test
def test_cmu_binary_bomb():
    exampletest_single("cmu_binary_bomb")  # 3m


@slow_test
def test_csaw_wyvern():
    exampletest_single("csaw_wyvern")  # 3m40s


@slow_test
def test_b01lersctf():
    exampletest_single("b01lersctf2020_little_engine")


def test_defcamp_r100():
...
def test_ekopartyctf2016_rev250():
    exampletest_single("ekopartyctf2016_rev250")


def test_ekopartyctf2016_sokohashv2():
...
def test_flareon2015_10():
    exampletest_single("flareon2015_10")


def test_flareon2015_2():
    exampletest_single("flareon2015_2")


@slow_test
def test_flareon2015_5():
    exampletest_single("flareon2015_5")  # 2m11s


@slow_test
def test_hitcon2017_sakura():
    exampletest_single("hitcon2017_sakura")  # 6m


def test_google2016_unbreakable_0():
    exampletest_single("google2016_unbreakable_0")


def test_google2016_unbreakable_1():
...
def test_defcon2016quals_baby_re():
    exampletest_single("defcon2016quals_baby-re")


def test_simple_heap_overflow():
    exampletest_single("simple_heap_overflow")


def test_csci_0a():
    exampletest_single("CSCI-4968-MBE/challenges/crackme0x00a")


def test_csci_1():
    exampletest_single("CSCI-4968-MBE/challenges/crackme0x01")


def test_csci_2():
    exampletest_single("CSCI-4968-MBE/challenges/crackme0x02")


def test_csci_3():
    exampletest_single("CSCI-4968-MBE/challenges/crackme0x03")


def test_csci_4():
    exampletest_single("CSCI-4968-MBE/challenges/crackme0x04")


def test_csci_5():
    exampletest_single("CSCI-4968-MBE/challenges/crackme0x05")


def test_insomnihack_aeg():
    exampletest_single("insomnihack_aeg")


def test_android_license():
...
def test_angry_reverser():
    exampletest_single("hackcon2016_angry-reverser")


def test_sharif7():
    exampletest_single("sharif7_rev50")
...
def test_unmapped_analysis():
    exampletest_single("unmapped_analysis")
...
def test_csgames2018():
    exampletest_single("csgames2018")


def test_java_crackme1():
    exampletest_single("java_crackme1")


def test_java_simple3():
    exampletest_single("java_simple3")


def test_java_simple4():
    exampletest_single("java_simple4")


def test_ictf2017_javaisnotfun():
    exampletest_single("ictf2017_javaisnotfun")


def test_java_androidnative1():
    exampletest_single("java_androidnative1")
...
def test_example_inclusion():
    to_test = subprocess.check_output(
        [
            "/bin/bash",
            "-c",
            "for c in $(find -name solve.py | cut -c 3-); do echo ${c%/solve.py}; done",
        ],
        cwd=_path("examples"),
    ).decode()
    ...
    with open(sourcefile, "r", encoding="utf-8") as fp:
        test_source = fp.read()
    example_tests = test_source[
        test_source.find("## BEGIN EXAMPLE TESTS") : test_source.find(
            "## END EXAMPLE TESTS"
        )
    ]
    ...
    if missing:
        raise Exception(
            "The following examples are not represented in the test corpus:\n"
            + "\n".join(missing)
        )
...
if __name__ == "__main__":
    ...
    for fname, func in globals().items():
        if (
            ...
            and fname != "test_example_inclusion"
        ):
            func()
